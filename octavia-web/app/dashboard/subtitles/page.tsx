...
import { FileVideo, Captions, Upload, Link as LinkIcon } from "lucide-react";
...
export default function SubtitleGenerationPage() {
    ...
    const handleFileSelect = (file: File | null) => {
        ...
        if (file && (file.type.startsWith("video/") || file.type.startsWith("audio/"))) {
            ...
            setError("Please select a valid media file (video or audio)");
        }
    };
    ...
    const handleGenerateSubtitles = async () => {
        if (!selectedFile && !mediaUrl) {
            setError("Please select a media file or paste a URL");
            return;
        }
        ...
        try {
            ...
            // Step 2: Create transcription job for subtitles
            const jobResponse = await fetch(`${API_BASE_URL}/api/v1/jobs/transcribe/create`, {
                method: "POST",
                headers: {
        ...
        <div className="space-y-8">
            ...
            {uploadMode === "file" ? (
                    ...
                    <div className="relative z-20 py-12 px-6">
                        <div className="flex flex-col items-center justify-center gap-3 text-center">
                            <div className="flex items-center justify-center w-16 h-16 rounded-2xl bg-primary-purple/10 border border-primary-purple/30 shadow-glow group-hover:scale-110 transition-transform">
                                <FileVideo className="w-8 h-8 text-primary-purple-bright" />
                            </div>
                            <div>
                                <h3 className="text-white text-lg font-bold mb-1 text-glow-purple">
                                    {selectedFile ? "✓ File Selected" : "Drop your media here"}
                                </h3>
                                {selectedFile ? (
                ...
                <motion.div className="glass-panel glass-panel-high p-6 mb-6">
                    <div className="flex flex-col gap-3">
                        <label htmlFor="media-url" className="text-white text-sm font-semibold">Media URL</label>
                        <input
                            id="media-url"
                            type="url"
                            title="Media URL"
                        ...
                        />
                        {mediaUrl && <p className="text-accent-cyan text-sm">✓ URL ready for processing</p>}
                    </div>
                </motion.div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <div className="glass-card p-4">
                    <label htmlFor="audio-lang" className="text-white text-sm font-semibold mb-2 block">Audio Language</label>
                    <select
                        id="audio-lang"
                ...
                <div className="glass-card p-4">
                    <label htmlFor="subtitle-format" className="text-white text-sm font-semibold mb-2 block">Subtitle Format</label>
                    <select
                        id="subtitle-format"
            ...
            <button
                ...
                disabled={isLoading || (!selectedFile && !mediaUrl)}
            ...
            >
                <div className="btn-border-beam-inner flex items-center justify-center gap-2 py-4 text-base">
                    <Captions className="w-5 h-5 group-hover:rotate-12 transition-transform duration-300" />
                    <span>{isLoading ? "Processing..." : "Generate Subtitles"}</span>
                </div>
            </button>
        </div>
    );
