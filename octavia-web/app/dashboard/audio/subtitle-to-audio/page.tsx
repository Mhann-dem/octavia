...
import { FileText, AudioWaveform, Sparkles, Upload, Link as LinkIcon } from "lucide-react";
...
export default function SubtitleToAudioPage() {
    ...
    const handleFileSelect = (file: File | null) => {
        ...
        if (file) {
            ...
            if (["srt", "vtt", "txt", "ass"].includes(ext || "")) {
                setSelectedFile(file);
                setSubtitleUrl("");
                setUploadMode("file");
            } else {
                setError("Please select a valid subtitle file (SRT, VTT, TXT, or ASS)");
            }
        }
    };
    ...
    const handleGenerateAudio = async () => {
        ...
        try {
            ...
            // Step 2: Create synthesis job
            const jobResponse = await fetch(`${API_BASE_URL}/api/v1/jobs/synthesize/create`, {
                method: "POST",
                headers: {
    ...
    return (
        <div className="space-y-8">
                        ...
                        <div className="flex flex-col items-center justify-center gap-3 text-center">
                            ...
                            <div>
                                <h3 className="text-white text-lg font-bold mb-1 text-glow-purple">
                                    {selectedFile ? "✓ File Selected" : "Drop your subtitle file here"}
                                </h3>
                                {selectedFile ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                ...
                <div className="glass-card p-4">
                    <label htmlFor="voice-gender" className="text-white text-sm font-semibold mb-2 block">Voice Gender</label>
                    <select
                        id="voice-gender"
            ...
            >
                <div className="btn-border-beam-inner flex items-center justify-center gap-2 py-4 text-base">
                    ...
                    <span>{isLoading ? "Processing..." : "Generate Audio"}</span>
                </div>
            </button>
        </div>
    );
